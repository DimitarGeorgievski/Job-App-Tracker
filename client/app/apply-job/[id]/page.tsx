...
export default function ApplyJobPage() {
  ...
  const stepOneData = useRef({ notes: "", phone: "" });
  ...
  const stepOneForm = useForm({
    ...
    validators: { onChange: stepOneApplySchema },
    onSubmit: async ({ value }) => {
      setServerError("");
      stepOneData.current = value;
      setStep(2);
    },
  });
  const stepTwoForm = useForm({
    ...
    validators: { onChange: stepTwoApplySchema },
    onSubmit: async ({ value }) => {
      setServerError("");
      try {
        const formData = new FormData();
        formData.append("jobId", String(Number(id)));
        formData.append("notes", stepOneData.current.notes);
        formData.append("phone", stepOneData.current.phone);
              ...
              >
                ...
                {step !== 3 && (
                  <div className="flex justify-end gap-4 mt-6">
                    ...
                    {step === 1 && (
                      <stepOneForm.Subscribe
                        selector={(s) => [s.canSubmit, s.isTouched]}
                      >
                        {([canSubmit, isTouched]) => (
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              stepOneData.current = stepOneForm.state.values;
                              setStep(2);
                            }}
